...
def check_field_length(field_name: str, max_length: int):  # Фабрика декораторов, проверяющая ограничение длинны max_length символов для поля field_name
    def decorator(f):
        def wrapper(*args, **kwargs):
            res = f(*args, **kwargs)
            if len(res) > max_length:
                raise ValueError(f"Длинна поля {field_name} превышает {max_length} символов")
            return res
        return wrapper
    return decorator
...
def get_rating() -> float:  # Функция получения поля rating
